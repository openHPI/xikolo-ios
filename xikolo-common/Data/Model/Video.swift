...
class Video : Content {
    ...
    func metadata() -> [AVMetadataItem] {
        ...
        if let course_item = self.item, let item = AVMetadataItem.item(AVMetadataIdentifier.commonIdentifierTitle.rawValue, value: course_item.title) {
            items.append(item)
        }
        if let item = AVMetadataItem.item(AVMetadataIdentifier.commonIdentifierDescription.rawValue, value: summary) {
            items.append(item)
        }
        return items
    }
